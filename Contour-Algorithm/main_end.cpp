...
vector <interval> Intervals_subtraction(vector<interval> arrA, vector<interval> arrB){
    ...
    while(i<arr1.size() && j<arr2.size()){
        if(arr1[i].bottom.x == arr2[j].bottom.x){
            ...
            // interval int1; // int1.bottom.x = arr2[j].bottom.x; // int1.top.x = min(arr1[i].bottom.x,arr2[j].top.x);
            j++; //ans.push_back(int1);
        }
    }
    if(j>=arr2.size()){
        while(i<arr1.size()){
...
// TO DO
interval union_intervals(ctree x_union){
    interval fin;
...
void intervals(edge h, stripe_contour s,vector <interval> &J){
    interval sx_union = union_intervals(s.tree);
