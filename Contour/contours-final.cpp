    ...
    for(int i=0; i < S.size(); i++){
        for(auto intv:J){
            if(interval_subset(S[i].y_inter,intv)){
                ...
                //S[i].tree->lson = new ctree(S[i].x_inter.bottom.x,0);
                //S[i].tree->rson = new ctree(S[i].x_inter.top.x,1);
                break;
            }
        }
...
vector<stripe_contour> concat(vector<stripe_contour> S1, vector<stripe_contour> S2, vector<cord> P, interval x_int){
    ...
    for(auto strp:S){
        ...
        strp.x_union = Intervals_union(st1.x_union,st2.x_union);
...
void intervals(edge h, stripe_contour s,vector <interval> &J){
    ...
    // if(uintervals.size()==0){
    //     return;
    // }
    vector<interval> tmp1; 
    tmp1.push_back(h.inter);
    // cout << "Edge interval " << h.inter.bottom.x << "," << h.inter.top.x << "\n";
    // cout << "Stripe intervals : \n";
    // for(auto intv:uintervals)
    //     cout << intv.bottom.x << "," << intv.top.x << "\n";
...
bool rect_comp(rectangle a, rectangle b){
    if(a.x_left.x<b.x_left.x) return true;
    if(a.x_right.x<b.x_right.x) return true;
    if(a.y_bottom.x<b.y_bottom.x) return true;
    if(a.y_top.x<b.y_top.x) return true;
    return false;
}
...
int main(){
    ...
    for(auto strp:S){
        ...
        cout << "*** Intervals in this stripe ***\n";
        for(auto intv:strp.x_union){
            cout << intv.bottom.x << "," << intv.top.x << "\n";
        } 
    }
    ...
    for(auto strp:S){
        ...
        print_tree(strp.tree);
        cout << "*****" << endl;
        ass.clear(); lefts.clear(); rights.clear();
    }
}